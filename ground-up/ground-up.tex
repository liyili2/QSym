...
A quantum operator is drawn in the following way in a quantum circuit.
...
Serial composition is given by composition of operators. In the matrix representation, this is matrix multiplication.
...
\end{tikzpicture}
\end{minipage}
  \caption{This is both $(F \otimes G) \circ (S \otimes T)$ and also $(F \circ S) \otimes (G \circ T)$}
\end{figure}
...
deterministic operator which takes in one bit and produces two outputs by duplicating the input bit.
\\
